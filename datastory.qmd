...
average_prices_1999 <- data1999 %>%
  group_by(flat_type) %>%
  summarise(Mean1999 = mean(resale_price))


average_prices_2022 <- data2022 %>%
  group_by(flat_type) %>%
  summarise(Mean2022 = mean(resale_price))
...
ggplot(long_data, aes(x = flat_type, y = AveragePrice, group = Year, color = Year)) +
  geom_line() +
  geom_point() +
  geom_text(aes(label = round(AveragePrice, 2)), hjust = 1, vjust = -1, check_overlap = TRUE, size = 3) +
  scale_y_continuous(labels = label_comma()) + 
  labs(title = "Average Resale Price Each Flat Type in 1999 and 2022 in SG Dollars",
       x = "Flat Type",
